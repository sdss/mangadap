...
def get_stellar_continuum_fom_maps(dapmaps):
    ...
    return extent, snrg, rms, frms, rchi2, chi_grw


def get_emission_line_data(daptype, dapmaps, dapcube):
    if not os.path.isfile(dapmaps):
        raise FileNotFoundError('{0} does not exist.'.format(dapmaps))
    if not os.path.isfile(dapcube):
        raise FileNotFoundError('{0} does not exist.'.format(dapcube))
...
def fom_maps(plt, ifu, snr, color, rms, frms, rchi2, chi_grw, extent=None,
             fit='sc', ofile=None):
...
def fit_residuals_muse(dapver, analysis_path, daptype, plt, ifu):
    ...
    extent, snrg, rms, frms, rchi2, chi_grw \
            = get_stellar_continuum_fom_maps(maps_file) # got rid of gmr here
    ofile = '{0}-maps.png'.format(ofile_root)
    fom_maps(plt, ifu, snrg, snrg, rms, frms, rchi2, chi_grw, extent=extent, fit='sc',
             ofile=ofile)
    ...
    ofile_root = os.path.join(plan_qa_dir, 'manga-{0}-{1}-LOGCUBE-{2}-el-fitqa'.format(
                                plt, ifu, daptype))
    wave, flux, error, model, snrg, rms, frms, rchi2 \
            = get_emission_line_data(daptype, maps_file, cube_file)
    ofile = '{0}-lambda.png'.format(ofile_root)
    ...
    ofile = '{0}-maps.png'.format(ofile_root)
    fom_maps(plt, ifu, anr, aob, rms, frms, rchi2, chi_grw, extent=extent, fit='el',
             ofile=ofile)
...
def main(args):
    ...
    for daptype in daptypes:
        fit_residuals(args.dapver, analysis_path, daptype, args.plate,
                      args.ifudesign)
