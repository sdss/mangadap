...
KEYWORDS:
   ...
   FRACTION: This keyword allows one to fix the ratio between the first two
    ...
    FTOL: Fractional tolerance for stopping the non-linear minimization.
...
KEYWORDS:
    ...
    FRACTION: This keyword allows one to fix the ratio between the first two
      ...
      - The remaining kinematic components (COMPONENT > 1) are left free, and
        this allows, for example, to still include gas emission line components.
    FTOL: Fractional tolerance for stopping the non-linear chi2 minimization.
        The default value is generally OK, but this can be useful for testing.
    GAS_COMPONENT: boolean vector, of the same size as COMPONENT, set to True
        where the given COMPONENT describes a gas emission line. If given, pPXF
        provides the pp.gas_flux and pp.gas_flux_error in output.
      - EXAMPLE: In the common situation where component = 0 are stellar
...
MODIFICATION HISTORY:
    ...
    V5.1.15: Updated documentation. Thanks to Peter Weilbacher (Potsdam) for
    ...
    V6.0.5: Consistently use new format_output() function both with/without
            the `linear` keyword. Added .status attribute.
            Changes suggested by Kyle B. Westfall (Santa Cruz).
            MC, Oxford, 21 February 2017
    V6.0.6: Added linear_fit() and nonlinear_fit() functions to better
    ...
    V6.4.1: linear_fit() does not return unused status any more, for
    ...
    V6.7.2: Included dunder names as suggested by Peter Weilbacher (Potsdam).
          - Fixed wrong .gas_reddening when mdegree > 0.
          - Improved formatting of documentation.
    V6.7.3: Plot wavelength in nm instead of Angstrom, following IAU rules.
          - Ensures each element of START is not longer than its MOMENTS.
          - Removed underscore from internal function names.
          - Included `ftol` keyword. MC, Oxford, 8 February 2018
