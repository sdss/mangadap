"""
Create a set of spatially binned spectra based on a provided DRP file.
...
*Class usage examples*::
    ...
    from mangadap.proc.BinnedSpectra import BinnedSpectra
    ...
    # Bin the spectra
    parfile = 'test.par'
    binningpar = SpectralBinningPar(parfile)
    binned_spectra = BinnedSpectra(binningpar)
...
class TplLibrary:
    ...
    Attributes:
        ...
        tplbm (BitMask): A BitMask object used to toggle mask values;
            see :func:`mangadap.util.bitmasks.TplLibraryBitMask`.
...
#        print('Reading: calling read_template_library.')
    ...
    def _define_library(self, library_key, dapsrc=None):
        ...
        Raises:
            KeyError: Raised if the selected keyword is not among the
                available list.
        """
        keys, paths, lsf_fwhm, vac, wlim, flim  = available_template_libraries(dapsrc)
        ...
        self.file_search = paths[indx][0]
        self.fwhm = lsf_fwhm[indx][0]
        self.invac = vac[indx][0]
        self.wave_limit = wlim[indx][0]
        self.flux_limit = flim[indx][0]
    ...
    def _build_raw_hdu(self, npix):
        ...
        :func:`mangadap.util.bitmasks.TplLibraryBitMask`.
...
#        pyplot.show()
    ...
    def process_template_library(self, velocity_offset=0.0, force=False):
        ...
        If not processed already, process the template library for use
        with fitting the DRP data.
