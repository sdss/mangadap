...
def spec_fit(dapf, binspec):
    ...
    resid = flux-smod
    pyplot.plot(wave, flux, color='blue')
    pyplot.plot(wave, smod, color='red')
    pyplot.plot(wave, resid, color='black')
    pyplot.show()
...
def resid_vs_err(dapf, binspec, window=100):
    ...
    smod = dapf.hdu['SMOD'].data[:,binspec]
    resid = flux-smod
    sigma = numpy.array([ numpy.sqrt(1.0/ivar) for ivar in dapf.hdu['IVAR'].data[:,binspec] if ivar > 0] )
    pyplot.plot(wave, flux, color='blue')
    pyplot.plot(wave, smod, color='red')
    pyplot.plot(wave, resid, color='black')
    pyplot.plot(wave, sigma, color='green')
    pyplot.show()
