...
;	    - Block 3:
;
;		Read the template spectra, match the resolution to the galaxy
;		data, match the pixel sampling to the galaxy data.  Limit the
;		spectral range of the galaxy spectral bins using the ranges set
...
;	- Block 3 should be done directly after block 1 if only one template
;	  library is used for all spatial binning results.
...
pro MANGA_DAP_0, $
	...
	MDAP_EXECUTION_SETUP, total_filelist, output_root_dir, tpl_library_keys, $
			      template_libraries, emission_line_parameters, $
			      absorption_line_parameters, signifier, bin_type, bin_par, $
			      w_range_sn, threshold_ston_bin, bin_weight_by_sn2, w_range_analysis, $
...
;	print, ''
	...
	MDAP_LOG_INSTANTIATE, output_dir, manga_dap_version, signifier, total_filelist, $
			      ...
			      velocity_initial_guess, velocity_dispersion_initial_guess, ell, pa, $
			      Reff, log_file_unit
...
;	    print, mask[0,0:100]
	...
	if indx[0] ne -1 then begin
		...
		if file_test(tpl_out_fits) eq 0 then begin 
		    ...
		    print, 'Trimming templates to ' + MDAP_STC(min(wave)) + 'A to ' + $
			    MDAP_STC(max(wave)) + 'A'
		    ...
		    ; Resample the templates to match the object spectra: On
		    ; input, tpl_wave expected to have the same size as tpl_flux
		    ; (wavelength defined separately for each spectrum); on
		    ; output, tpl_wave and tpl_sres are single vectors common to
		    ; ALL template spectra.  Sampling is forced to match galaxy
		    ...
		    ; TODO: Write useful information to the header!
		    ...
		    SXADDPAR, header, 'VDAPTPL', mdap_tpl_lib_setup_version, $
			      'mdap_tpl_lib_setup version'
...
;	for i=0,n_plans-1 do begin
	i = 0
	    ...
	    ; TODO: If previous file exists, read it!  Otherwise ...
	    ...
	    print, 'Log file unit: ', log_file_unit
	    get_lun, unit
	    print, 'GOT UNIT: ', unit
	    free_lun, unit

	    ; Write some basics of the DRP fits files
	    ; TODO: Can free memory for variables if they won't be used again!
	    print, 'BASIC DATA'
...
;stop
	    ...
	    ;-------------------------------------------------------------------------------
	    ; Spatial binning
	    ; if execution_plan[i].overwrite eq 1 or (it's found) then
	    ...
	    printf, log_file_unit, '[INFO] Type of spatial binning: ', execution_plan[i].bin_type
	    printf, log_file_unit, '[INFO] Spatial bining version: ' + mdap_spatial_binning_version
...
;	    fits_info, execution_plan[i].ofile
