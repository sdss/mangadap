...
pro mdap_spectral_fitting,galaxy,noise,loglam_gal,templates,loglam_templates,velscale,$
     ...
     wavelength_output,best_fit_model,galaxy_minus_ems_fit_model,best_template,best_template_LOSVD_conv,reddening_output,residuals,$
...
if n_elements(extra_inputs) ne 0 then begin
   for i = 0, n_elements(extra_inputs)-1 do begin
       ...
       if not keyword_set(quiet) then  print, 'setting variable '+extra_inputs[i]
   endfor
endif
...
FOR i = 0, sz[2]-1 DO BEGIN  ;loop over all the spectra
   ...
   start = [star_kin_starting_guesses[i,0],star_kin_starting_guesses[i,1],gas_kin_starting_guesses[i,0],gas_kin_starting_guesses[i,1]]

   if i gt 0 and keyword_set(use_previos_guesses) then begin
      start = [sol[0],sol[1],sol[7],sol[8]]
   endif
...
if min(noise_) eq max(noise_) then error[0:5] = error[0:5] * sqrt(sol[6]) ; If the error vector is flat (i.e. errors are not reliable), I rescale the formal errors for sqrt(chi2/dof), as instructed by mpfit and ppxf.
   ...
   ; plot,exp(loglam_gal), galaxy_,title=string(i),xrange=[5100,6800]
