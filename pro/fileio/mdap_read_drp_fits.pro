...
;
;    ivar dblarr[N][T]
;		Inverse variances of the spectra. TODO: Will need to accommodate
;		covariance matrices.
;		
;    mask dblarr[N][T]
;		Bad pixel mask.
...
;
;    cdelt1 double
;		Spatial sampling along x direction (arcsec/pixel).
;
;    cdelt2 double
;		Spatial sampling along y direction (arcsec/pixel).  
;
; OPTIONAL OUTPUT:
;    version string
...
PRO MDAP_READ_DRP_FITS,$
		file, header, flux, ivar, mask, wave, sres, skyx, skyy, cdelt1, cdelt2, $
	...
	MDAP_DRP_FILETYPE, flux, type			; Determine/Confirm the file type
	...
	if type eq 'CUBE' then begin
	    ...
	    cdelt1=SXPAR(header,'CDELT1')		; Read the pixel size from the header
	    ...
	    cdelt1=0.5					; TODO: Assign a fixed pixel size
	    cdelt2=0.5
	endelse

	MDAP_ONSKY_XY_ARCS2CENTER, header, skyx, skyy	; Convert to arcseconds wrt targetted object
